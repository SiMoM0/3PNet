...
def get_train_augmentations(config):
    ...
    # remove unlabeled points (for customize log scale polar grid)
    list_of_transf.append(tr.RemoveUnlabeled())
    ...
    for aug_name in config["augmentations"].keys():
        ...
        elif aug_name == "instance_cutmix" or aug_name == "polarmix":
            # Do nothing here, directly handled in semantic kitti dataset
            continue
        else:
            raise ValueError("Unknown transformation")
        # add jitter augmentation (does not seem so useful for now)
        #list_of_transf.append(tr.Jitter(rate=0.3))
...
def get_datasets(config, args):
    ...
    train_dataset = DATASET(
        ...
        polarmix=config["augmentations"]["polarmix"],
        **kwargs,
    )
...
def distributed_training(gpu, ngpus_per_node, args, config):
    ...
    def convert_activation(model, act):
        for child_name, child in model.named_children():
            if isinstance(child, nn.ReLU):
                setattr(model, child_name, act)
            else:
                convert_activation(child, act)

    # list of activation functions: nn.ReLU, nn.LeakyReLU, nn.SiLU, nn.Hardswish, nn.GELU
    #convert_activation(model, nn.Hardswish())

    cuda0 = torch.device('cuda:0')

    # ckpt = torch.load("./logs/data/ckpt_best.pth", map_location="cuda:0")
    # try:
